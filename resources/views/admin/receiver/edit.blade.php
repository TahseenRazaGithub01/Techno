              ...
              <div class="email-wrapper row remain-height pos-r scrollable bgc-white">
                <div class="email-content open no-inbox-view">
                  <div class="email-compose">
                    ...
                    <form method="post" action="{{ route('admin.receiver.update') }}" enctype="multipart/form-data" class="email-compose-body">
                      @csrf
                      
                      <input type="hidden" name="id" value="{{ $record->id }}">
                      <h4 class="c-grey-900 mB-20">Update Receiver </h4>
                      <div class="send-header">
					  
						<div class="form-row">
						  <div class="form-group col-md-6">
							<label for="inputEmail4">Receiver Name</label>
							<input type="text" required="required" class='form-control' name="receiver_name" value="{{ $record->receiver_name }}" placeholder="Complete Receiver Name">
						  </div>
						  <div class="form-group col-md-6">
							<label for="inputPassword4">Bank Name</label>
							<input type="text" required="required" class='form-control' id="bank_name" name="bank_name" value="{{ $record->bank_name }}" placeholder="Enter Bank Name">
						  </div>
						  <div class="form-group col-md-6">
							<label for="inputPassword4">Account Number</label>
							<input type="text" required="required" class='form-control' id="number" name="account_number" value="{{ $record->account_number }}" placeholder="Enter Account Number">
							&nbsp;<span id="errmsg"></span>
						  </div>						  						  
